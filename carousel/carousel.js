...
var animateCarousel = function(duration, repeat) {
    ...
    $(".imagegroup ul").animate({ marginLeft: repeat * -600 },
                                duration,
                                function(){
...
var buildControlRow = function() {
    ...
    for (var i = 0; i < images.length; i++) {
        ...
        var ctrlImg = $("<img>")
            .attr("src", "/assets/images/aria_javascript/circle"+(i+1)+"_grey.png")
...
var buildFocusHandler = function(srcUrl) {
    return function(event) {
        focusOnElement(srcUrl); event.preventDefault(); };
}
...
var pauseCarousel = function() {
    clearInterval(timer);
    $("#playPauseButton")
        .attr("src", $("#playPauseButton").attr("src").replace("pause", "play"))
...
var playCarousel = function() {
    timer = setInterval(function() { animateCarousel(500, 1); }, 4500);
    $("#playPauseButton")
        .attr("src", $("#playPauseButton").attr("src").replace("play", "pause"))
